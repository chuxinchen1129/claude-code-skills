...
所有 Skill 必须统一存储在：
```
DaMiShuSystem-main-backup/skills/[skill-name]/
```

访问入口：
```
~/.claude/skills  ──[软链接]──>  DaMiShuSystem-main-backup/skills/
```

**软链接机制**：
- `~/.claude/skills` 是指向主技能库的软链接
- 任何在主技能库的修改都会自动同步到访问入口
- 创建 Skill 时直接写入主技能库位置，无需操作软链接
...
### 目录结构
**输出路径约定**: DaMiShuSystem-main-backup/skills/[skill-name]/
