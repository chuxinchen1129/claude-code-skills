...
| 项目 | 说明 |
|------|------|
| **触发方式** | 说"报告喵"、"用baogaomiao"、"阅读PDF"等 |
| **处理内容** | PDF/Word/PPT/网页文档 |
| **输出格式** | 中文标题 + 英文标题 + 小红书笔记 + 6张截图 + 封面 |
| **发送位置** | 自动发送到飞书 |
| **默认路径** | `~/Library/Mobile Documents/com~apple~CloudDocs/家人共享/报告喵` |
| **核心要求** | **必须按照3步输出格式，内容必须来自PDF原文** |

## 🎯 如何在Claude Code中使用（重要）

### 方式一：直接对话触发（推荐）
直接对Claude Code说以下任一指令即可触发：
- "报告喵" - 使用默认报告喵文件夹的最新PDF
- "用baogaomiao" - 同上
- "阅读这个PDF" - 处理指定PDF文件
- "总结这份文档" - 生成小红书笔记
- "读一下这个报告" - 分析并生成笔记
...
- "报告喵 /path/to/folder" - 处理指定文件夹中的最新PDF
- "用baogaomiao /path/to/file.pdf" - 同上

### 方式三：默认路径处理
如果只说"报告喵"或"用baogaomiao"（不带路径），会自动使用：
```
...
当触发报告喵skill后，会自动执行以下流程：

```
用户触发 → 读取PDF → 生成截图 → 提取内容 → 生成小红书笔记 → 发送飞书
```
...
### 👤 用户使用指南
- [🎯 如何在Claude Code中使用](#-如何在claude-code中使用重要) ← **从这里开始**
- [🚀 完整工作流程](#-完整工作流程)
- [⚠️ 使用前必读](#-⚠️-使用前必读)
- [📋 输出格式规范](#-输出格式规范)
- [🔧 故障排查](#-故障排查)
...
报告喵skill有严格的3步输出格式，**必须严格遵守**：

### 第1步：中文标题
- **必须以"🎯2026"开头**
- 格式：`🎯2026 [主标题] —— [副标题]`
- 主标题：一眼看出是报告（如：行业趋势报告、数据经营白皮书、发展洞察报告）
- 副标题：一句话提炼核心价值/亮点（10-20字）
- 示例：`🎯2026母婴连锁数据经营报告 —— 逆势增长背后的经营密码`

### 第2步：英文标题
- 对应中文标题的英文翻译
- 保持简洁有力
- 示例：`2026 How Maternal & Child Chains Defied the Odds`
...
**内容提取原则（重要）：**
1. **品类优先**：优先提取产品/品类相关信息
2. **数据解读**：数据要说明品类趋势和市场机会
3. **对比突出**：品类对比、竞品对比、行业对比
4. **结论导向**：每段先说品类洞察，再给数据支撑
5. **字数控制**：正文总字数控制在800字左右
6. **避免聚焦**：减少股权收购、净利润、股价等财务指标

**不要编造内容！**所有数据和分析必须来自PDF原文。
...
- 使用全角竖线 `｜` 分隔3行内容
- 格式：`年份｜主题｜报告性质`
- 示例：`2026｜中国移动互联网｜流量年度报告`
- **重要**：不管原标题是哪一年，封面统一显示为2026年
- 生成的封面只显示3行中文标题（英文标题保留在底部）
