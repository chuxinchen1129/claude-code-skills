...
SHARED_CONFIG = PROJECT_ROOT / "03_WUXIN_CONTENT/assets/config.yaml"

# 备用路径：如果共享配置不存在，尝试加载旧路径
FALLBACK_ASSETS = Path("/Users/echochen/Desktop/DaMiShuSystem-main-backup/Wuxin_Zenoasis_Content_Project/01_Brand_Assets_Library/assets.yaml")
    ...
    for config_path in possible_paths:
        try:
            if config_path.exists():
                ...
                # 从 config.yaml 中提取品牌信息
                if "brand_guidelines" in assets:
                    return {
                        "name": "悟昕Zenoasis",
                        "role": "首席内容主理人",
                        "tone": ", ".join(assets["brand_guidelines"].get("tone_words", ["温暖治愈", "科学循证", "优雅克制"])),
                        "values": "拒绝焦虑贩卖，主张长期主义"
                    }
                # 或从旧格式 assets.yaml 中提取
                elif "brand_profile" in assets:
    ...
    for config_path in possible_paths:
        try:
            if config_path.exists():
                ...
                if "brand_guidelines" in assets:
                    bg = assets["brand_guidelines"]
                    return {
                        "tone_control": bg.get("tone_words", []),
                        "structure_control": [],
                        "negative_constraints": bg.get("forbidden_words", [])
                    }
                elif "writing_guidelines" in assets:
