...
HOOK_TEMPLATES = {
    "数据冲击型": [
        "画面：数据大字弹出 | 音效：数字音 | 台词：{data}的人都有这个问题你知道吗",
        "画面：对比画面 | 音效：惊讶 | 台词：原来我们一直都做错了"
    ],
    "痛点直击型": [
        "画面：痛点特写（黑眼圈、疲惫） | 音效：沉重 | 台词：{pain}？真的太难了",
        "画面：主角痛苦表情 | 音效：叹息 | 台词：{pain}，那种感觉谁懂"
    ],
    "反常识型": [
        "画面：❌打叉动画 | 音效：错误提示 | 台词：{common_sense}",
        "画面：大字报风格 | 音效：冲击音 | 台词：{surprise_fact}"
    ]
}

# 3-10s 痛点深挖模板
PAIN_TEMPLATES = [
    "画面：{pain场景演绎} | 台词：就是{pain_detail}那种感觉，越{emotion}越睡不着",
    "画面：情绪化表达 | 台词：明明{condition}，但{pain}真的太难受了",
    "画面：具体场景细节 | 台词：{pain}的场景，{emotion}感满满"
]

# 10-20s 原理展现模板
PRINCIPLE_TEMPLATES = [
    "画面：科技感演示 | 台词：原来不是{misconception}，是{truth}才关键",
    "画面：数据可视化 | 台词：{selling}这个技术，真的能看到问题所在",
    "画面：产品使用场景（产品模糊） | 台词：监测之后才知道，{truth}才是关键"
]

# 20-30s 爽感结尾模板
ENDING_TEMPLATES = [
    "画面：状态对比（使用前后） | 台词：现在{result}！{cta}",
    "画面：笑容、精神饱满 | 台词：那种睡饱的感觉，真的不一样！{cta}",
    "画面：数据报告 | 台词：数据看到改变，{cta}"
