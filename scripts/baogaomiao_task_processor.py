...
class BaogaomiaoTaskProcessor:
    ...
    def process_task(self, task_file):
        ...
        try:
            ...
            if send_result['success']:
                ...
                # 提取核心品类（从关键词或PDF内容中）
                categories = self._extract_categories(pdf_text)
    ...
    def _generate_chinese_title(self, pdf_text):
        ...
        for line in lines[:10]:
            ...
            # 检查是否包含标题关键词
            if any(keyword in line for keyword in ['行业', '品牌', '报告', '深度', '高端', '分析', '研究']):
                # 移除股票代码等干扰
                title = re.sub(r'（\d+）|证券研究报告·|新股研究报告·|消费电子|\s+', '', line)
                title = title.strip()

                if len(title) >= 6:  # 主标题至少6个字
                    if not main_title:
                        main_title = title
                    elif not subtitle and len(title) <= 20:  # 副标题10-20字
                        subtitle = title
                    break

        # 第二步：生成符合格式的标题
        if main_title:
            return f"🎯2026 {main_title} —— {subtitle}"
        else:
            # 如果没有提取到标题，使用默认值
            return f"🎯2026 PDF内容总结 —— 深度分析"
    ...
    def _extract_categories(self, pdf_text):
        """从PDF文本中提取核心品类
        ...
        # 返回前3个品类
        return '、'.join(found_categories[:3])
            ...
            while True:
                ...
                for task_file in tasks:
                    ...
                    if result['success']:
                        ...
                        logger.error(f"  ✗ 任务 {result.get('task_id', 'unknown')} 失败: {result.get('error', '')}")
